...
6. **`signal run --live`** — **failed 则短路** portfolio→ledger；skipped（非调仓）继续
7. **`portfolio build --live`** — 非调仓日 `require_signal_as_of` → hold skipped
8. **`risk review --drafts`**
9. **`execution run --approved`** — 每单 CLI 内立即过账
10. **`ledger post --unposted`** — 兜底
11. `ops_monitor.notify`
