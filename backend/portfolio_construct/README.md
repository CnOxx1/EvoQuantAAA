...
| 目标持仓腿 | `portfolio_target_position` | 整手股数 + 目标市值；含 `can_buy`/`can_sell` |
| 资本配额 | `strategy_capital_alloc` | 可选登记；`build --live` 多策略时切分账户 NAV |
...
| data_process | `../data_process/README.md` | 价格/`can_buy`/`can_sell` | 经库读 processed 日线 |
...
## 边界
- 做：读 **committed** `signal_batch` 权重 → 剔不可买/缺价 → 权重归一 → 整手 sizing → 写 **draft**；同日同账户幂等；多策略按 `strategy_capital_alloc`（缺省等权）切分账户 NAV；**live 路径非调仓日 hold**（`signal_trade_date != as_of` → skipped）。
...
- `processed_equity_bar_1d`（**未复权 `close`** 优先 + `can_buy`/`can_sell`，回看 60 日）
- `cost_params.lot_size`；默认账本权益，或 `--fixed-nav --nav`
- 可选 `strategy_capital_alloc`
...
- sizing/成交价口径：`close`（未复权），缺价才退 `adj_close`
- `(strategy_version, as_of_date, account_id)` 活跃态唯一（迁移 `029`）
- 资本配额：迁移 `030`；未登记则 runnable 集合等权且归一
