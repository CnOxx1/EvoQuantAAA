...
| execution_run | oltp | execution | ledger, ops_monitor | execution_id | OMS；adapter∈{paper,broker_stub,live_gated}；`run_kind` |
| execution_adapter_params | ref_data | migrations 种子 | execution | kind | 适配器启用/是否允许成交/是否要求 live 环境 |
...
| execution_pending_event | oltp | execution | ops_monitor | event_id | 续撮审计 |
